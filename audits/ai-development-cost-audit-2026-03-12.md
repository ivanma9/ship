...
| ---------------------------------- | ------------ | --------------------- | ----------------------- | ------------------------------------------------------------------ |
| AI provider integrations in code   | `rg`         | `rg -n "OpenAI        | openai                  | Anthropic                                                          |
| AI request entry points            | `rg`         | `rg -n "analyze-plan  | analyze-retro           | /api/ai/status" api/src/routes api/src/openapi web/src/components` |
| Request/token caps per call path   | `rg` + `sed` | `rg -n "RATE_LIMIT    | MAX_CONTENT_TEXT_LENGTH | max_tokens                                                         |
| Historical token/accounting fields | `rg`         | `rg -n "prompt_tokens | completion_tokens       | input_tokens                                                       |
| Coding-agent cost observability    | `rg`         | `rg -n "cursor        | copilot                 | claude code                                                        |
